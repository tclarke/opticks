...
#ifndef ALGORITHM_PATTERN_H
#define ALGORITHM_PATTERN_H
...
public:
   ...
   AlgorithmReporter(const AlgorithmReporter &reporter) : 
      mpProgress(reporter.mpProgress) {}
   ~AlgorithmReporter();
...
private:
   ...
   public:
      ...
      int mWeight;
      int mWeightSum;
      Stage(std::string message, int weight, int prevWeight = 0);
...
protected:
   ...
   void displayThresholdResults(RasterElement *pRasterElement, ColorType color,
      PassArea passArea, double firstThreshold, double secondThreshold, Opticks::PixelOffset offset=Opticks::PixelOffset());
...
public:
   AlgorithmPattern(RasterElement* pRasterElement, 
      Progress* pProgress, bool interactive, const BitMask* pRoi);
...
private:
   ...
   virtual bool needToRunAlgorithm() { return true; }
};
