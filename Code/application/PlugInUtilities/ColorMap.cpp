...
ColorMap::ColorMap() : mIsDefault(false), mpGradient(NULL)
{
   resetToDefault();
}

ColorMap::ColorMap(const ColorMap& colorMap) :
   mName(colorMap.mName), mTable(colorMap.mTable), mIsDefault(colorMap.mIsDefault), mpGradient(NULL)
{
}

ColorMap::ColorMap(const std::string& filename) :
   mIsDefault(false), mpGradient(NULL)
{
...
bool ColorMap::tableIsValid(const std::vector<ColorType>& table)
{
   ...
   {
      if (table[i].isValid() == false) return false;
   }
...
bool ColorMap::deserialize (QIODevice &io)
{
   ...
   if (io.readLine(buffer, BUFFER_SIZE) == -1) return false;
   ...
   int version = 0;
   if (io.readLine(buffer, BUFFER_SIZE) == -1) return false;
   if (sscanf(buffer, "Version%d", &version) != 1) return false;
   ...
   char buffer1[BUFFER_SIZE], buffer2[BUFFER_SIZE], buffer3[BUFFER_SIZE], buffer4[BUFFER_SIZE], buffer5[BUFFER_SIZE];
   if (io.readLine(buffer, BUFFER_SIZE) == -1) return false;
   if (version == 3)
   {
   ...
   if (strcmp(buffer1, "Index") != 0) return false;
   if (strcmp(buffer2, "Red") != 0) return false;
   if (strcmp(buffer3, "Green") != 0) return false;
   if (strcmp(buffer4, "Blue") != 0) return false;
   ...
   for (int i=0; ; ++i)
   {
      int count=0, index=0, red=0, green=0, blue=0, alpha=255;
      if (io.readLine(buffer, BUFFER_SIZE) == -1) break;
   ...
   if (version == 1 && colorMap.size() != VERSION_ONE_TABLE_SIZE) return false;

   if (!ColorMap::tableIsValid(colorMap)) return false;
...
{
   if (!ColorMap::tableIsValid(table)) return false;
...
std::vector<ColorType> ColorMap::tableFromGradient (const Gradient &gradient)
{
   ...
   {
      while (i > gradient.mControls[nextPrimary].mPosition || gradient.mControls[prevPrimary].mPosition == gradient.mControls[nextPrimary].mPosition)
      {
         ++prevPrimary;
         ++nextPrimary;
...
bool ColorMap::serialize(QIODevice &io) const
{
   ...
   for (i = 0; i < mTable.size(); ++i)
   {
      if (version == 3)
      {
         out << i << "\t" << mTable[i].mRed << "\t" << mTable[i].mGreen << "\t" << mTable[i].mBlue << "\t" << mTable[i].mAlpha << endl;
      }
      else
      {
...
{
   if (mpGradient == NULL) return false;
   QTextStream out(&io);
