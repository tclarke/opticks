...
public:
   ...
   XmlRpcStructParam() : XmlRpcParam("struct") {}
   XmlRpcStructParam(const XmlRpcStructParam &other) : XmlRpcParam(other), mStruct(other.mStruct) {}
   ~XmlRpcStructParam()
   {
   ...
   const XmlRpcParam *operator[](const QString &key) const { return mStruct[key]; }
   type::size_type size() const { return mStruct.size(); }
   type::const_iterator begin() const { return mStruct.begin(); }
   type::const_iterator end() const { return mStruct.end(); }

   virtual bool isValid() const { return !mStruct.isEmpty(); }
